  ...
  const fetchProducts = async () => {
    try {
      ...
      const errorMessage = error.response 
        ? `Erro ${error.response.status}: ${error.response.data?.message || 'Erro ao conectar com a API'}`
        : error.message || 'Erro ao carregar produtos. Verifique se o backend está rodando.';
      toast.error(errorMessage);
      setProducts([]); // Garantir que products está vazio em caso de erro
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        getApiUrl(`products/search?q=${searchTerm}`)
      );
